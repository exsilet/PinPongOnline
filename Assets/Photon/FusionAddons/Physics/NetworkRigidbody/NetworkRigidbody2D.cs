...
namespace Fusion.Addons.Physics
{
    ...
    public class NetworkRigidbody2D : NetworkRigidbody<Rigidbody2D, RunnerSimulatePhysics2D>
    {
        ...
        protected override NetworkRigidbodyFlags GetRBFlags(Rigidbody2D rb)
        {
            ...
            if (rb.isKinematic)
            {
                flags |= NetworkRigidbodyFlags.IsKinematic;
            }

            if (rb.IsSleeping())
            {
                flags |= NetworkRigidbodyFlags.IsSleeping;
            }
        ...
        {
            // Linear threshold
            if (((Vector2)data.LinearVelocity).sqrMagnitude >
                Physics2D.linearSleepTolerance * Physics2D.linearSleepTolerance)
            {
                return false;
            }
